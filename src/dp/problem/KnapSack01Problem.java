...
public class KnapSack01Problem {
	...
	static int knapsackRec(int[] wt, int[] val, int W, int n) {
		...
		if (wt[n] <= W) {
			return Math.max(val[n] + knapsackRec(wt, val, W - wt[n], n - 1), knapsackRec(wt, val, W, n - 1));
		} else {
	...
	static int knapsackUsingMemo(int[] wt, int[] val, int W, int n, int[][] storage) {
		...
		if (wt[n - 1] <= W) {
			storage[n][W] = Math.max(val[n - 1] + knapsackUsingMemo(wt, val, W - wt[n - 1], n - 1, storage),
